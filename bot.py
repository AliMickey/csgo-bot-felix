...
async def showList(ctx, type):
    ...
    for playerId, values in playersDict.items():
        playersList.append({playerId: values})
    
    splitLength = 8
    playersList = [playersList[x:x+splitLength] for x in range(0,len(playersList),splitLength)]

    for split in playersList:
        playerEmbed = discord.Embed(title = embedTitle, colour = Color.purple())
        for player in split:
            for playerId, values in player.items():
